...
<table class="table text-center">
    ...
    <tbody>
        <tr>
            ...
            <td><button type="button" class="btn btn-primary" data-toggle="modal" data-target="#notesModal_{{$logbook->id}}">notes</button></td>
            <td><button type="button" class="btn btn-primary mb-2" data-toggle="modal" data-target="#updateModal_{{$logbook->id}}">edit</button></td>
            <td>    <form action="{{route('logbook-delete', $logbook->id)}}" method="POST">
                        @csrf
                        @method('DELETE')
